...
def _evaluate_all_qubo_energies_no_jit(X_matrix, h_dense, J_dense):
    """
    Executes on GPU/CPU via XLA.
    X_matrix: shape (M, N) - The valid bitstrings
    h_dense: shape (N,)
...
def _get_valid_bitstrings_matrix_no_jit(params: BasicParams, logger: logging.Logger) -> tuple[np.ndarray, np.ndarray]:
    num_qubits = params.num_qubits
    wire_offsets = params.wire_offsets
    seq_positions = params.seq_positions
    rotamer_counts = params.rotamer_counts
...
def extract_lowest_energy_bitstrings_no_jit(h_linear, J_quadratic, logger, epsilon, params: BasicParams):
    X_matrix, indices = _get_valid_bitstrings_matrix_no_jit(params, logger)
    h_dense, J_dense = build_dense_qubo(h_linear, J_quadratic, params.num_qubits, params.wire_offsets)

    energies = _evaluate_all_qubo_energies_no_jit(
        jnp.array(X_matrix),
        jnp.array(h_dense),
        jnp.array(J_dense)
    )
