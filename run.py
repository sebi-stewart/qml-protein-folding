...
def run_one_config(config: RunConfig, seed_versions: list[int], cost_hamiltonian, basic_params,
    ...
    logger.info(f"===== Starting batched GPU run (Layers: {layers}) =====")
    ...
    logger.info(f"===== Run Complete time taken = {time_taken:5.3f} seconds | {time_taken / len(seed_versions):5.3f} per run =====\n")
