...
def evaluate_singular_pyrosetta_energy(bitstring: list[int], pose,
    ...
    for seq in seq_positions:
        # print(f"Evaluating seq {seq} - bitstring controls this position, checking rotamer bits {wire_offsets[seq]} to {wire_offsets[seq] + rotamer_counts[seq] - 1}")
    ...
    for seq in all_seq:
        ...
        # print(f"Setting fixed rotamer for seq {seq} - bitstring does not control this position")
...
def exhaustively_evaluate_all_conformations(unique_bitstrings: list[list[int]], original_pose, scorefxn, residue_library: dict[int, TrackedResidue], params: BasicParams):
    conformations = []
    for nd_bitstring in unique_bitstrings:
        bitstring = list(map(int, nd_bitstring))
        # print(list(map(int, bitstring)))
        new_pose = evaluate_singular_pyrosetta_energy(bitstring, original_pose, residue_library, params)
        biological_energy = np.float64(scorefxn(new_pose))
        conformations.append(
            RescoringConformation(
...
def main(logger: logging.Logger, fac: TestInstanceFactory, results_file: str = "0/phase2_5_to_10_qubits/AF-5PTI_52_55_4_12_layers.npz"):
    ...
    one_body, two_body, pose, residue_library, ig, rot_sets, scorefxn = extract_one_body_energies_from_instance(inst, phase2_rescoring_logger)
    logger.info(f"Extracted residues: {residue_library.keys()} --- {residue_library}")
    logger.info(f"Extracted one-body energies: {one_body.keys()} --- {one_body}")

    sample_func, basic_params = get_sample_function_for_phase_2(phase2_rescoring_logger, one_body, two_body)
    ...
    logger.info(f"Evaluating PyRosetta energies for unique bitstrings... sequences: {basic_params.seq_positions}, wire offsets: {basic_params.wire_offsets}, rotamer counts: {basic_params.rotamer_counts}")
    ...
    for seed, conf in best_conf_per_seed.items():
        ...
        logger.debug(
            f"Seed {seed}: Best Bitstring: {conf.bitstring}, Biological Energy: {conf.biological_energy:.4f}, Energy Difference: {conf.energy_diff:.4f}")
        if abs(conf.energy_diff) < epsilon_value:
            logger.debug(
                f"Seed {seed} is too close to the original pose (Energy difference {conf.energy_diff:.4f} < {epsilon_value}), it's a tie.")
            tie_count += 1
        elif conf.energy_diff < 0:
            logger.debug(
                f"Seed {seed} has a better conformation than the original pose (Energy difference {conf.energy_diff:.4f} < 0), it's a success!")
            win_count += 1
        else:
            logger.debug(
                f"Seed {seed} has a worse conformation than the original pose (Energy difference {conf.energy_diff:.4f} > 0), it's a failure.")
            loss_count += 1
